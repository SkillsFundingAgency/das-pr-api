...
public class RemovePermissionsCommandValidatorTests
{
    ...
    private readonly Mock<IProviderReadRepository> _providersReadRepositoryMock = new Mock<IProviderReadRepository>();
    [Test]
    ...
    [Test]
    public async Task Validate_Ukprn_Returns_NoUkprn_ErrorMessage()
    {
        var sut = new RemovePermissionsCommandValidator(_accountLegalEntityReadRepositoryMock.Object, _providersReadRepositoryMock.Object);
        var result = await sut.TestValidateAsync(new RemovePermissionsCommand { Ukprn = null, AccountLegalEntityId = 1, UserRef = Guid.NewGuid() });
        result.ShouldHaveValidationErrorFor(q => q.Ukprn)
            .WithErrorMessage(RemovePermissionsCommandValidator.UkprnValidationMessage);
    }
