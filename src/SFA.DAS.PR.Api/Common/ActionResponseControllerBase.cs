...
namespace SFA.DAS.PR.Api.Common
{
    ...
    public abstract class ActionResponseControllerBase : ControllerBase
    {
        ...
        protected IActionResult GetPostResponse<T>(ValidatedResponse<T> response, string postMethodName, object? routeParameters) where T : class
        {
            if (response.IsValidResponse)
            {
                return new CreatedAtActionResult(postMethodName, ControllerName, routeParameters, response.Result);
            }
